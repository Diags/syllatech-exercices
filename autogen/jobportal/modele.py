...
`OpenAIChatCompletionClient` vient d'`autogen_ext`, **un paquet séparé** que
`autogen-agentchat` ne tire pas. Le cours le nomme et l'installe ; ce projet
...
class ClientFactice(ChatCompletionClient):
    ...
    async def create(self, messages: Sequence[LLMMessage], *, tools=(),
        ...
        # TODO : si des outils sont disponibles et qu'aucun retour n'est encore arrive, rendre un CreateResult dont le content est une liste de FunctionCall (finish_reason="function_calls"). Sinon, repondre en texte. Quatre tests le verifient.
    ...
    def model_info(self) -> ModelInfo:
        # `function_calling=True` n'est pas décoratif : à False AVEC des
        # outils, `AssistantAgent` lève dès la construction. Sans `tools=`,
        # il répond normalement et n'appelle jamais rien — c'est ce cas-là
        # qui est silencieux. Un test le mesure.
    ...
    def _repondre(self, messages: Sequence[LLMMessage]) -> str:
        ...
        # TODO : si le prompt est celui d'un selecteur, rendre EXACTEMENT un nom de participant. Une autre reponse fait retomber AutoGen sur l'orateur precedent avec un simple avertissement : l'equipe tourne, mal. Trois tests le verifient.
        pass
