...
Le cours écrit `from autogen_ext.models.openai import OpenAIChatCompletionClient`
sans le dire : **`autogen-ext` est un paquet séparé**, que
`pip install autogen-agentchat` ne tire pas. Et il demande une clé.
...
- Passez `function_calling=False` dans `model_info` : l'agent devient muet, sans erreur.
