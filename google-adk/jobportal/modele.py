...
class ModeleFactice(BaseLlm):
    ...
    async def generate_content_async(self, llm_request: LlmRequest,
        ...
        # TODO : si des outils sont disponibles et qu'aucun retour n'est encore arrive, rendre une LlmResponse dont le content porte une types.Part(function_call=types.FunctionCall(...)). Sinon, repondre en texte. Cinq tests le verifient.
...
def _choisir(outils: list[dict], texte: str,
    ...
    # TODO : ne deleguer QUE si aucun outil metier n'est disponible. Choisir « transfer_to_agent » systematiquement fait rebondir la question entre coordinateur et expert jusqu'au RecursionError. Deux tests le verifient.
    pass
...
def _destinataire(instruction: str, demande: str) -> str:
    ...
    if not candidats:
        return ""
    # TODO : choisir le sous-agent dont la DESCRIPTION partage le plus de mots avec la demande. C'est la description — jamais l'instruction — qu'ADK donne au coordinateur pour decider. Trois tests le verifient.
