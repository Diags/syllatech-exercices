...
def appliquee(config: Config) -> tuple[Config, list[Ignoree]]:
    ...
    """
    # >>> depart: rendre la configuration APPLIQUEE et la liste des cles jetees. Hors des portees privilegiees, cinq cles sont ignorees : filesystem.disabled, network.strictAllowlist, network.tlsTerminate, credentials.allowPlaintextInject, et toute entree credentials en mode mask (envVars ET files). Un mask ignore ne devient pas un deny : l'entree DISPARAIT, donc la vraie valeur reste lisible. Cinq tests le verifient.
    #     return config, []
    if config.portee in PORTEES_PRIVILEGIEES:
        return config, []

    ignorees: list[Ignoree] = []
    fs = dict(config.filesystem)
    reseau = dict(config.network)
    secrets = {c: (list(v) if isinstance(v, list) else v)
               for c, v in config.credentials.items()}

    if fs.pop("disabled", False):
        ignorees.append(Ignoree(
            "filesystem.disabled", config.portee,
            CLES_PRIVILEGIEES["filesystem.disabled"],
            "l'isolation des fichiers reste ACTIVE — ici, la bonne surprise"))

    if reseau.pop("strictAllowlist", False):
        ignorees.append(Ignoree(
            "network.strictAllowlist", config.portee,
            CLES_PRIVILEGIEES["network.strictAllowlist"],
            "les hotes hors liste sont DEMANDES, pas refuses"))

    if reseau.pop("tlsTerminate", False):
        ignorees.append(Ignoree(
            "network.tlsTerminate", config.portee,
            CLES_PRIVILEGIEES["network.tlsTerminate"],
            "le proxy ne dechiffre pas : tout masquage devient impossible"))

    if secrets.pop("allowPlaintextInject", False):
        ignorees.append(Ignoree(
            "credentials.allowPlaintextInject", config.portee,
            CLES_PRIVILEGIEES["credentials.allowPlaintextInject"],
            "aucune injection dans une requete en clair"))

    # ⚠️ Le plus dangereux des cinq. Un « mask » ignoré ne devient pas un
    # « deny » : l'entrée DISPARAÎT. La variable garde donc sa vraie valeur, et
    # les commandes sandboxées la lisent en entier — alors que la configuration
    # dit « mask » et qu'on la relit en pensant être couvert.
    for famille, champ in (("envVars", "name"), ("files", "path")):
        gardees, jetees = [], []
        for entree in secrets.get(famille, []):
            (jetees if entree.get("mode") == "mask" else gardees).append(entree)
        if jetees:
            secrets[famille] = gardees
            noms = tuple(e.get(champ, "?") for e in jetees)
            ignorees.append(Ignoree(
                f"credentials.{famille}[].mode=mask", config.portee,
                CLES_PRIVILEGIEES["credentials[].mode=mask"],
                "« " + ", ".join(noms) + " » n'est ni masque NI refuse : "
                "l'entree disparait, et la vraie valeur reste lisible dans le "
                "bac a sable",
                noms))

    return (replace(config, filesystem=fs, network=reseau, credentials=secrets),
            ignorees)
    # <<<
...
def _plus_specifique(chemin: str,
    ...
    """
    # >>> depart: rendre (autorise, prefixe) de la regle la PLUS LONGUE qui couvre le chemin, ou None si aucune ne le couvre. Ni « deny d'abord » ni « allow d'abord » : la specificite tranche, et c'est ce qui rend le resultat independant de l'ordre des entrees. Un test le verifie sur les 720 permutations de six regles.
    #     return None
    trouvees = [(len(prefixe), autorise, prefixe)
                for prefixe, autorise in regles if _sous(chemin, prefixe)]
    if not trouvees:
        return None
    _, autorise, prefixe = max(trouvees)
    return autorise, prefixe
    # <<<
...
def protege(chemin: str, travail: str = "/projet", maison: str = "/home/moi",
    ...
    """
    # >>> depart: rendre le motif protege qui couvre ce chemin, ou None. Quatre groupes : PROTEGES_REMONTEE dans le dossier de travail ET ses parents, PROTEGES_A_LA_RACINE et PROTEGES_DEPOT_NU dans le dossier de travail seul, PROTEGES_DEPOT_NU_SI_EXISTANTS seulement si le nom est dans « existants », PROTEGES_MAISON sous le dossier personnel. Attention : un prefixe de CHAINE n'est pas un prefixe de CHEMIN — passer par _sous. Vingt-cinq tests le verifient.
    #     return None
    cible = _normaliser(chemin, travail, maison)

    for motif in PROTEGES_REMONTEE:
        for base in _remontee(travail):
            if _sous(cible, f"{base}/{motif}".replace("/**", "")):
                return motif

    for motif in PROTEGES_A_LA_RACINE + PROTEGES_DEPOT_NU:
        if _sous(cible, f"{travail}/{motif}".replace("/**", "")):
            return motif

    for motif in PROTEGES_DEPOT_NU_SI_EXISTANTS:
        nom = motif.removesuffix("/**")
        if nom in (existants or set()) and _sous(cible, f"{travail}/{nom}"):
            return f"{motif} (present a la racine)"

    for motif in PROTEGES_MAISON:
        if _sous(cible, f"{maison}/{motif}".replace("/**", "")):
            return f"~/{motif}"
    return None
    # <<<
