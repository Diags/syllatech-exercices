...
def verifier(config: Config) -> list[Souci]:
    ...
    masques_fichiers = [e for e in effective.credentials.get("files", [])
                        if e.get("mode") == "mask"]
    # TODO : signaler un mask sans network.tlsTerminate. Le proxy doit dechiffrer pour reconnaitre la sentinelle et y remettre la vraie valeur ; sans terminaison TLS il ne peut pas, donc la sentinelle part telle quelle et l'authentification rate. Rien ne fuit, mais rien ne marche. Deux tests le verifient.
    pass
