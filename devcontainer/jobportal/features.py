...
def construire(demandees: dict[str, dict[str, Any]],
    ...
    # >>> depart: accumuler les Features et resoudre `dependsOn` recursivement
    #     return [Feature(i, o or {}, _manifeste_de(i, index))
    #             for i, o in demandees.items()]
    accumulateur: list[Feature] = []
    vues: set[tuple] = set()
    file = [(i, o if isinstance(o, dict) else {}, True)
            for i, o in demandees.items()]

    while file:
        identifiant, options, demandee = file.pop(0)
        feature = Feature(identifiant, options,
                          _manifeste_de(identifiant, index), demandee)
        if feature.cle() in vues:
            continue
        vues.add(feature.cle())
        accumulateur.append(feature)
        for dure in feature.dures:
            options_dure = (feature.manifeste.get("dependsOn") or {}).get(dure)
            file.append((dure,
                         options_dure if isinstance(options_dure, dict) else {},
                         False))
    return accumulateur
    # <<<
...
def prioriser(features: list[Feature], ordre_impose: list[str]) -> None:
    ...
    graphe.
    """
    # >>> depart: donner a chaque Feature nommee une priorite de n - index
    #     return
    n = len(ordre_impose)
    rangs = {nom_qualifie(nom): i for i, nom in enumerate(ordre_impose)}
    for feature in features:
        feature.priorite = n - rangs[feature.qualifie] \
            if feature.qualifie in rangs else 0
    # <<<
...
def ordonner(features: list[Feature]) -> tuple[list[Feature], list[Tour]]:
    ...
    # >>> depart: trier par tours — dependances satisfaites, puis priorite maximale
    #     return list(file), []
    numero = 0
    while file:
        numero += 1
        tour = [f for f in file
                if exigences[id(f)] <= installes]
        if not tour:
            restants = ", ".join(str(f) for f in file)
            raise CycleDeDependances(
                f"tour {numero} : aucune Feature installable — "
                f"dependance circulaire parmi {restants}")
        priorite_max = max(f.priorite for f in tour)
        retenues = _tri_stable([f for f in tour if f.priorite == priorite_max])
        journal.append(Tour(numero, list(tour), retenues, priorite_max))
        for feature in retenues:
            ordre.append(feature)
            installes.add(feature.qualifie)
            file.remove(feature)
    return ordre, journal
    # <<<
