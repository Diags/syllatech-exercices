...
def deroulement(config: Config) -> list[Etape]:
    ...
    """
    # >>> depart: marquer comme finies les commandes jusqu'a `waitFor` INCLUSE
    #     return [Etape(n, OU[n], QUAND[n], config.brut[n], True)
    #             for n in COMMANDES if n in config.brut]
    attendue = config.attend
    rang_attendu = COMMANDES.index(attendue) if attendue in COMMANDES else -1
    etapes = []
    for rang, nom in enumerate(COMMANDES):
        if nom not in config.brut:
            continue
        etapes.append(Etape(nom, OU[nom], QUAND[nom], config.brut[nom],
                            rang <= rang_attendu))
    return etapes
    # <<<
