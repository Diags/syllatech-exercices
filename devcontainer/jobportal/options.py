...
def nom_de_variable(option: str) -> str:
    ...
    # >>> depart: appliquer les trois substitutions, dans l'ordre de l'amont
    #     return option.upper()
    return _TETE.sub("_", _NON_MOT.sub("_", option)).upper()
    # <<<
...
def env(manifeste: dict[str, Any],
    ...
    # >>> depart: exporter TOUTES les options declarees, defaut compris
    #     return {nom_de_variable(n): _valeur(v) for n, v in passees.items()}
    sortie: dict[str, str] = {}
    for nom, corps in declarees.items():
        defaut = corps.get("default") if isinstance(corps, dict) else None
        valeur = passees.get(nom, defaut)
        if valeur is not None:
            sortie[nom_de_variable(nom)] = _valeur(valeur)
    return sortie
    # <<<
