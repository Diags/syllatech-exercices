...
public class GestionnaireErreurs extends ResponseEntityExceptionHandler {
    ...
    ProblemDetail entrepriseInconnue(EntrepriseInconnue erreur) {
        // >>> depart: rendre un ProblemDetail 422, avec un titre, un type sous BASE, et le nom en propriete
        //     return ProblemDetail.forStatus(HttpStatus.INTERNAL_SERVER_ERROR);
        // ⚠️ UNPROCESSABLE_CONTENT, et non UNPROCESSABLE_ENTITY. La RFC 9110
        // a renomme le code 422, et Spring Framework 7 a suivi : l'ancien nom
        // existe encore mais il est deprecie, et `HttpStatus.valueOf(422)`
        // rend desormais la NOUVELLE constante. Un test qui comparait les
        // deux enumerations echouait donc sur « 422 attendu, 422 obtenu ».
        var probleme = ProblemDetail.forStatusAndDetail(
                HttpStatus.UNPROCESSABLE_CONTENT, erreur.getMessage());
        probleme.setTitle("Entreprise inconnue");
        probleme.setType(URI.create(BASE + "entreprise-inconnue"));
        // Une propriete supplementaire : le client peut agir dessus sans
        // avoir a analyser la phrase francaise du champ `detail`.
        probleme.setProperty("entreprise", erreur.nom());
        return probleme;
        // <<<
    }
