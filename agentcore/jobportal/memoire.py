...
def extraire(message: Message, strategies: set[str]) -> list[Souvenir]:
    ...
    inventée lui-même.
    """
    # >>> depart: extraire les souvenirs d'un message. Seuls les messages de l'UTILISATEUR comptent : extraire les mots de l'assistant lui ferait croire qu'il a APPRIS ce qu'il vient de dire, et au troisieme tour il defend une preference qu'il a inventee. Appliquer PREFERENCES si « preference » est active, FAITS si « fait » l'est. Quatre tests le verifient.
    #     return []
    if message.role != "USER":
        return []

    plat = message.texte.lower()
    souvenirs = []
    if "preference" in strategies:
        for motif, _ in PREFERENCES:
            trouve = re.search(motif, plat)
            if trouve:
                souvenirs.append(Souvenir("preference", trouve.group(0).strip()))
    if "fait" in strategies:
        for motif, etiquette in FAITS:
            trouve = re.search(motif, plat)
            if trouve:
                souvenirs.append(
                    Souvenir("fait", f"{etiquette} : {trouve.group(1)}"))
    return souvenirs
    # <<<
