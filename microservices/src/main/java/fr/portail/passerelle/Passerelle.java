...
public class Passerelle {
    ...
    public RouterFunction<ServerResponse> routeDesOffres() {
        ...
        return route("offres")
                ...
                // >>> depart: reecrire le chemin public /api/offres vers /offres, que le service expose
                //     // sans cela, le service recoit /api/offres et rend 404
                .before(rewritePath("/api/offres", "/offres"))
                // <<<
    ...
    public RouterFunction<ServerResponse> routeStricte() {
        return route("offres-strict")
                ...
                .filter(lb("offres-service"))
                // >>> depart: compter les reponses 5xx comme des echecs, sinon le circuit ne s'ouvrira jamais
    ...
    public RouterFunction<ServerResponse> routeProtegee() {
        return route("offres-protegees")
                ...
                .filter((requete, suivant) -> {
                    // >>> depart: refuser en 401 une requete sans jeton « Bearer », AVANT d'atteindre le service
                    //     // sans ce controle, la passerelle relaie tout
                    String jeton = requete.headers()
                            .firstHeader("Authorization");
                    if (jeton == null || !jeton.startsWith("Bearer ")) {
                        REFUS.incrementAndGet();
                        return ServerResponse.status(401)
                                .body("jeton absent ou mal forme");
                    }
                    // <<<
