...
class Cluster:
    ...
    def _avancer_la_mise_a_jour(self, deploiement: Deploiement,
        ...
        """
        # >>> depart: ajouter tant que maxSurge le permet, retirer tant que maxUnavailable le permet
        #     pass
        voulus = deploiement.voulus
        plafond = voulus + deploiement.strategie.surge(voulus)
        plancher = max(0, voulus - deploiement.strategie.indisponible(voulus))

        total = sum(rs.voulus for rs in self.replicasets.values()
                    if rs.deploiement == deploiement.nom)
        prets = self.disponibles(deploiement.nom)

        avance = False
        if courant.voulus < voulus and total < plafond:
            # On AJOUTE tant que le plafond de surge le permet.
            courant.voulus += 1
            avance = True
        else:
            # ⚠️ Et on ne RETIRE que si le plancher tient encore APRES le
            # retrait. Quand `maxUnavailable` vaut 0, cela veut dire : tant
            # qu'un nouveau pod n'est pas PRET, on ne touche a rien. Le
            # deploiement s'arrete plutot que de descendre sous le plancher
            # — et le service continue de repondre.
            #
            # Nuance qui compte : retirer un pod qui n'est PAS pret ne coute
            # rien a la disponibilite. C'est ce qui permet a un
            # `rollout undo` de faire disparaitre immediatement les pods
            # casses, sans attendre quoi que ce soit.
            for ancien in sorted(anciens, key=lambda rs: rs.revision):
                if ancien.voulus <= 0:
                    continue
                siens = [pod for pod in self.pods.values()
                         if pod.proprietaire == ancien.nom]
                cout = 0 if any(not pod.pret for pod in siens) else 1
                if prets - cout >= plancher:
                    ancien.voulus -= 1
                    avance = True
                    break
        ...
        if avance:
            ...
            deploiement.depuis_le_dernier_progres += 1
            if deploiement.depuis_le_dernier_progres >= deploiement.delai_de_progression \
                    and not deploiement.condition:
                deploiement.condition = "ProgressDeadlineExceeded"
                self._journal(
                    f"deployment/{deploiement.nom} : ProgressDeadlineExceeded "
                    f"— la mise a jour n'avance plus depuis "
                    f"{deploiement.delai_de_progression}s")
        # <<<
    ...
    def _placer(self, pod: Pod) -> str | None:
        ...
        """
        # >>> depart: choisir le noeud le moins charge qui a la place demandee
        #     return sorted(self.noeuds)[0] if self.noeuds else None
        candidats = []
        for nom in sorted(self.noeuds):
            cpu, memoire = self.reste(nom)
            occupation = sum(1 for p in self.pods.values() if p.noeud == nom)
            if pod.cpu_demande <= cpu and pod.memoire_demandee <= memoire:
                candidats.append((-memoire, -cpu, occupation, nom))
        if not candidats:
            return None
        return sorted(candidats)[0][3]
        # <<<
    ...
    def endpoints(self, service: str) -> list[Pod]:
        ...
        # >>> depart: rendre les pods PRETS dont les etiquettes correspondent
        #     return []
        selecteur = self.services[service].selecteur
        return sorted(
            (pod for pod in self.pods.values()
             if pod.pret and _correspond(pod.etiquettes, selecteur)),
            key=lambda p: p.nom)
        # <<<
