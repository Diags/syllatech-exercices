...
def cosinus(a: list[float], b: list[float]) -> float:
    ...
    """
    # >>> depart: ecrire la similarite cosinus. Un test verifie l'identite ||a-b|| au carre = 2 - 2 cos sur des vecteurs normalises.
    #     return 0.0
    na = math.sqrt(sum(x * x for x in a))
    nb = math.sqrt(sum(x * x for x in b))
    return produit_scalaire(a, b) / (na * nb) if na and nb else 0.0
    # <<<
