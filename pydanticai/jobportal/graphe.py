...
# >>> depart: cabler le graphe : start -> collecter, puis une decision() qui branche sur le TYPE rendu — .branch(match(Assez).to(analyser)) et .branch(match(TropPeu).to(renoncer)) — et les deux issues vers end_node. Retirez ensuite validate_graph_structure=False du build. Quatre tests le verifient, dont un qui exige que les DEUX branches soient reellement empruntees.
...
constructeur.add_edge(analyser, constructeur.end_node)
constructeur.add_edge(renoncer, constructeur.end_node)
