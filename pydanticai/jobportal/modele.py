...
def conseiller(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
    ...
    """
    # >>> depart: au premier tour, appeler le premier outil disponible (ToolCallPart). Aux tours suivants, lire le dernier ToolReturnPart et construire la sortie A PARTIR DE LUI. Un test exige que les offres citees existent toutes dans la base.
    #     return ModelResponse(parts=[TextPart("a")])
    retours = [p for m in messages for p in m.parts if isinstance(p, ToolReturnPart)]

    if not retours and info.function_tools:
        outil = info.function_tools[0]
        arguments = {nom: _valeur(nom, schema, messages)
                     for nom, schema in outil.parameters_json_schema
                     .get("properties", {}).items()}
        return ModelResponse(parts=[ToolCallPart(outil.name, arguments)])

    # On a le retour de l'outil : on répond EN S'APPUYANT DESSUS.
    contenu = retours[-1].content if retours else []
    if info.output_tools:
        return ModelResponse(parts=[ToolCallPart(
            info.output_tools[0].name, _sortie(info, contenu))])
    return ModelResponse(parts=[TextPart(_resume(contenu))])
    # <<<
