...
def agent_strict(m=None) -> Agent[Deps, Recommandation]:
    ...
    async def les_offres_existent(ctx: RunContext[Deps],
        ...
        # >>> depart: lever ModelRetry si la sortie cite une offre absente de la base, en NOMMANT les offres fautives dans le message (il est renvoye au modele, qui recommence). Trois tests le verifient, dont un qui exige que le message contienne « n'existent pas ».
        #     return sortie
        reelles = await ctx.deps.db.offres(ctx.deps.candidat_id)
        inventees = [o for o in sortie.offres_citees if o not in reelles]
        if inventees:
            # ModelRetry renvoie le message AU MODÈLE, qui recommence. Ce
            # n'est pas une exception pour l'appelant : c'est une correction.
            raise ModelRetry(
                f"Ces offres n'existent pas : {inventees}. "
                f"Utilise uniquement celles rendues par l'outil.")
        return sortie
        # <<<
