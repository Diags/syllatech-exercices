...
def construire():
    ...
    def superviseur(state: EtatEquipe) -> dict:
        ...
        """
        # >>> depart: dépiler « plan », poser le nom retiré dans « courant », déléguer avec CONSIGNES[nom], et journaliser. Rendre courant vide quand le plan l'est.
        #     return {"courant": ""}
        if not state["plan"]:
            return {"courant": ""}
        suivant, *reste = state["plan"]
        # On délègue avec une CONSIGNE, pas en laissant le spécialiste
        # interpréter la conversation précédente. C'est ce qui distingue une
        # équipe d'un groupe : chacun sait ce qu'on attend de lui.
        return {"plan": reste, "courant": suivant,
                "messages": [HumanMessage(content=CONSIGNES[suivant])],
                "journal": [f"superviseur → {suivant}"]}
        # <<<
