...
class VeilleFlow(Flow):
    ...
    def evaluer(self, rapport=None):
        ...
        # >>> depart: rendre « publier » si le rapport porte au moins une tendance, « resoumettre » sinon. Le seuil est du CODE : il ne varie pas d'une execution a l'autre et se teste sans modele. Trois tests le verifient, dont un qui exige que les DEUX branches soient empruntees.
        #     return "publier"
        tendances = getattr(rapport, "tendances", []) or []
        return "publier" if len(tendances) >= 1 else "resoumettre"
        # <<<
