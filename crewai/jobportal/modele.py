...
class ModeleFactice(BaseLLM):
    ...
    def call(self, messages, tools=None, callbacks=None, available_functions=None,
        ...
        # TODO : recuperer les noms d'outils. CrewAI ne les passe PAS par « tools » : il les decrit DANS LE PROMPT, sous « Tool Name: ... ». Chercher uniquement dans `tools` donne un agent qui n'appelle jamais rien, sans la moindre erreur. Trois tests le verifient.
        noms = []
        if noms:
...
def _schema_du_prompt(nom: str, prompt: str) -> list[str]:
    ...
    # TODO : extraire les champs « required » du schema JSON de l'outil. Inventer les noms d'arguments fait echouer l'action, et l'erreur revient au modele comme une observation ordinaire : la reponse finale cite alors le message d'erreur, ce qui ressemble a un resultat.
    return []
