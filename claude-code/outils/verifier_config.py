...
def _verifier_commande(racine: Path, ou: str, commande: str) -> list[Souci]:
    ...
    for morceau in shlex.split(commande, posix=False):
        ...
        if not re.search(r"\.(py|sh|js|mjs|ts)$", morceau):
            continue
        # TODO : verifier que le script existe, apres avoir retire le prefixe $CLAUDE_PROJECT_DIR/ (ou ${CLAUDE_PROJECT_DIR}/). Le hook survit au script : il echoue alors a chaque declenchement, et l'echec d'un hook ne remonte pas toujours. Deux tests le verifient.
        continue
    return soucis
...
def verifier_mcp(racine: Path, reglages: dict) -> list[Souci]:
    ...
    # TODO : signaler chaque regle « mcp__<serveur>__<outil> » dont le SERVEUR est absent de .mcp.json. La regle est acceptee, elle a l'air de proteger, et elle ne protege rien — aucun avertissement ne le dit.
    pass
    return soucis
...
def verifier_claude_md(racine: Path) -> list[Souci]:
    ...
    # TODO : signaler chaque « npm run X » de CLAUDE.md absent de package.json. CLAUDE.md est relu a CHAQUE session : une commande fausse y est repetee a l'agent des centaines de fois, et il la lance des centaines de fois. Trois tests le verifient.
    pass
