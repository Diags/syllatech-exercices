...
public record CreationOffre(
        // >>> depart: valider a la FRONTIERE — un titre non vide et borne, une pile parmi java/devops/front, un salaire plausible. La coherence metier, elle, reste au service
        //     String titre,
        //     String pile,
        //     int salaireEnKiloEuros,
        //     long entrepriseId) {
        @NotBlank @Size(max = 160) String titre,
        @NotBlank @Pattern(regexp = "java|devops|front",
                           message = "pile inconnue : java, devops ou front")
        String pile,
        @Min(20) @Max(300) int salaireEnKiloEuros,
        @Min(1) long entrepriseId) {
    // <<<
}
