...
public final class Reconciliation {
    ...
    public static Rendu parCle(List<Element> avant, List<Element> apres) {
        ...
        List<Operation> operations = new ArrayList<>();
        // >>> depart: apparier par CLE, pas par position — un element retrouve garde son nœud du DOM et son etat ; et un element deja dans le bon ordre relatif ne bouge PAS (heuristique du dernier indice place)
        //     return new Rendu(List.of());
        // ⚠️ L'HEURISTIQUE DU « DERNIER INDICE PLACE », celle que React
        // applique vraiment. On parcourt la NOUVELLE liste ; un element dont
        // l'ancien rang est superieur a tous ceux deja vus est deja au bon
        // endroit — il n'y a rien a deplacer. Seul celui qui « recule » bouge.
        //
        // C'est ce qui fait qu'une insertion en tete ne coute qu'UNE
        // operation : les six autres gardent leur rang relatif.
        int dernierRangPlace = -1;
        for (int rang = 0; rang < apres.size(); rang++) {
            Element nouveau = apres.get(rang);
            Element ancien = anciens.remove(nouveau.cle());
            if (ancien == null) {
                operations.add(Operation.CREER);
                continue;
            }
            if (!ancien.texte().equals(nouveau.texte())) {
                operations.add(Operation.METTRE_A_JOUR);
            }
            int rangAncien = rangsAnciens.get(nouveau.cle());
            if (rangAncien < dernierRangPlace) {
                operations.add(Operation.DEPLACER);
            } else {
                dernierRangPlace = rangAncien;
            }
        }
        // Ce qui reste dans `anciens` n'est plus rendu.
        for (int restant = 0; restant < anciens.size(); restant++) {
            operations.add(Operation.SUPPRIMER);
        }
        return new Rendu(List.copyOf(operations));
        // <<<
