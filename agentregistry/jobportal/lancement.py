...
def derivee(paquet: dict[str, Any]) -> tuple[str, str]:
    ...
    # >>> depart: deriver la commande de l'origine — npx pour npm, uvx pour pypi, ENTRYPOINT pour oci
    #     return "", "a completer"
    type_ = origine.get("type")
    identifiant = origine.get("identifier") or ""
    if type_ == "npm":
        sous = origine.get("npm") or {}
        return f"npx -y {identifiant}@{sous.get('version', '')}", "derivee"
    if type_ == "pypi":
        sous = origine.get("pypi") or {}
        return f"uvx {identifiant}=={sous.get('version', '')}", "derivee"
    if type_ == "oci":
        return f"(ENTRYPOINT de {identifiant})", "image"
    return "", f"origine {type_!r} inconnue"
    # <<<
