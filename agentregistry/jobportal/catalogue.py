...
class Catalogue:
    ...
    def identite_de(doc: Document) -> Identite:
        ...
        """
        # >>> depart: poser les deux defauts du serveur — espace et etiquette
        #     return Identite(doc.type, doc.espace, doc.nom, doc.etiquette)
        espace = doc.espace or ESPACE_PAR_DEFAUT
        etiquette = (doc.etiquette or ETIQUETTE_PAR_DEFAUT) \
            if etiquetable(doc.type) else ""
        return Identite(doc.type, espace, doc.nom, etiquette)
        # <<<
    ...
    def resoudre(self, doc: Document) -> list[ErreurDeChamp]:
        ...
        """
        erreurs = []
        # >>> depart: signaler chaque reference dont la cible n'est pas au catalogue
        #     return erreurs
        for chemin, cible, _ in self.references_de(doc):
            if cible not in self.objets:
                erreurs.append(ErreurDeChamp(
                    chemin, f"reference pendante : {cible} est introuvable"))
        # <<<
        return erreurs
