...
public final class Journal {
    ...
    public static String decrire(Evenement evenement) {
        // >>> depart: un switch exhaustif sur les quatre cas, avec des motifs de record — sans `default`
        //     return evenement.toString();
        return switch (evenement) {
            case Evenement.Deposee(LocalDate date, var candidat, var offre) ->
                    "%s — %s postule sur %s (%s)".formatted(
                            date, candidat.nom(), offre.reference(), offre.intitule());
            case Evenement.Triee(LocalDate date, boolean retenue, String motif) ->
                    "%s — tri automatique : %s (%s)".formatted(
                            date, retenue ? "retenue" : "ecartee", motif);
            case Evenement.Entretien(LocalDate date, String qui, int note) ->
                    "%s — entretien avec %s, note %d/20".formatted(date, qui, note);
            case Evenement.Decision(LocalDate date, var issue, String commentaire) ->
                    "%s — decision : %s (%s)".formatted(
                            date, issue.name().toLowerCase(java.util.Locale.ROOT),
                            commentaire);
        };
        // <<<
    }
