...
class Passerelle:
    ...
    async def traiter(self, jwt: str, alias: str, question: str) -> Appel:
        ...
        """
        # >>> depart: enchainer (1) verifier le jeton, (2) retrouver la cle virtuelle de l'equipe dans le coffre, (3) autoriser aupres du registre, (4) appeler le proxy, (5) imputer le cout reel et journaliser. Chaque echec leve un Refuse avec son code HTTP, et laisse une trace — un refus non journalise est un incident invisible. Onze tests le verifient.
        #     raise Refuse(500, "a ecrire")
        try:
            jeton = verifier(jwt, self.secret_jwt)
        except JetonInvalide as erreur:
            self._tracer("(inconnue)", alias, 401)
            raise Refuse(401, str(erreur)) from erreur

        cle = self.coffre.get(jeton.equipe)
        if cle is None:
            # L'utilisateur est authentifié, mais son équipe n'a pas de clé.
            # C'est 403, pas 401 : le client ne doit pas croire qu'il suffit
            # de se reconnecter.
            self._tracer(jeton.equipe, alias, 403)
            raise Refuse(403, f"aucune cle virtuelle pour « {jeton.equipe} »")

        try:
            self.registre.autoriser(cle, alias)
        except BudgetDepasse as erreur:
            self._tracer(jeton.equipe, alias, 429)
            raise Refuse(429, str(erreur)) from erreur
        except (CleInconnue, ModeleInterdit) as erreur:
            self._tracer(jeton.equipe, alias, 403)
            raise Refuse(403, str(erreur)) from erreur

        appel = await self.proxy.appeler(alias, question)
        self.registre.imputer(cle, appel.cout)
        self.journal.append(Trace(
            equipe=jeton.equipe, alias=appel.alias,
            modele_reel=appel.modele_reel, cout=appel.cout,
            jetons=appel.jetons, latence_ms=appel.latence_ms,
            bascule=appel.bascule))
        return appel
        # <<<
