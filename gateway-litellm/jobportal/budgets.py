...
class Registre:
    ...
    def autoriser(self, cle: str, alias: str) -> CleVirtuelle:
        ...
        # >>> depart: refuser (1) une cle inconnue ou revoquee — CleInconnue ; (2) un alias hors de la liste de la cle — ModeleInterdit, qui rendra 403 car il ne passera JAMAIS ; (3) un budget epuise — BudgetDepasse, qui rendra 429 car il repassera a la periode suivante ; (4) un depassement de rpm sur la minute glissante. Ne rien imputer ici : le cout n'est pas encore connu. Sept tests le verifient.
        #     return self.cles[cle]
        virtuelle = self.cles.get(cle)
        if virtuelle is None:
            raise CleInconnue("cle virtuelle inconnue")
        if virtuelle.revoquee:
            raise CleInconnue("cle revoquee")

        self._peut_etre_nouvelle_periode(virtuelle)

        if virtuelle.modeles and alias not in virtuelle.modeles:
            raise ModeleInterdit(
                f"« {alias} » n'est pas dans les modeles autorises de cette "
                f"cle ({', '.join(virtuelle.modeles)})")
        if virtuelle.depense >= virtuelle.budget_max:
            raise BudgetDepasse(
                f"budget epuise : {virtuelle.depense:.5f} $ sur "
                f"{virtuelle.budget_max:.5f} $")
        if virtuelle.rpm is not None and self._dans_la_minute(virtuelle) >= virtuelle.rpm:
            raise BudgetDepasse(f"rpm depasse : {virtuelle.rpm}/min")
        return virtuelle
        # <<<
