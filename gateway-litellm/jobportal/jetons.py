...
def verifier(jeton: str, secret: str) -> Jeton:
    ...
    # >>> depart: refuser (1) tout algorithme autre que HS256 — l'algorithme ATTENDU est celui du serveur, jamais celui que le jeton annonce, sinon « alg: none » passe ; (2) une signature fausse, comparee avec compare_digest pour ne pas fuir sa longueur par le temps de reponse ; (3) un jeton expire ; (4) un jeton sans claim « team ». Huit tests le verifient.
    #     raise JetonInvalide("a ecrire")
    # ⚠️ L'algorithme attendu est celui du SERVEUR. Lire `entete["alg"]` pour
    # choisir la vérification revient à laisser le jeton décider comment on le
    # vérifie — c'est la faille « alg: none ».
    if entete.get("alg") != ALGORITHME:
        raise JetonInvalide(f"algorithme refuse : {entete.get('alg')!r}")

    attendue = hmac.new(secret.encode(), f"{entete_b64}.{charge_b64}".encode(),
                        hashlib.sha256).digest()
    # `==` sur des octets s'arrête au premier différent : le temps de réponse
    # renseigne alors sur le nombre d'octets justes. `compare_digest` est à
    # temps constant.
    if not hmac.compare_digest(attendue, _deb64(signature_b64)):
        raise JetonInvalide("signature invalide")

    if time.time() >= charge.get("exp", 0):
        raise JetonInvalide("jeton expire")
    if not charge.get("team"):
        raise JetonInvalide("aucune equipe dans le jeton")

    return Jeton(sujet=charge.get("sub", ""), equipe=charge["team"],
                 expire_le=charge["exp"])
    # <<<
