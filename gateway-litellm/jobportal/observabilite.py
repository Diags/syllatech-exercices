...
def brancher(sonde: Sonde) -> Sonde:
    ...
    # >>> depart: enregistrer la sonde ET debrancher les precedentes. Reassigner litellm.callbacks ne suffit pas : litellm en derive des listes internes qu'il ne reconstruit pas, donc l'ancienne sonde continue de tout recevoir. Purger LISTES_DERIVEES de nos propres Sonde, sans toucher aux rappels internes de litellm. Un test le verifie, et outils/mesurer_doublon.py le mesure.
    #     litellm.callbacks = [sonde]; return sonde
    for nom in LISTES_DERIVEES:
        derivee = getattr(litellm, nom, None)
        if isinstance(derivee, list):
            derivee[:] = [c for c in derivee if not isinstance(c, Sonde)]
    litellm.callbacks = [sonde]
    return sonde
    # <<<
