...
function faireCirculer(donnees, flux, items, options = {}) {
  ...
  const visiter = (nom, entrants, vus) => {
    ...
    if (noeud && noeud.type === 'n8n-nodes-base.if') {
      // >>> depart: aiguiller les items sur les DEUX sorties du IF — index 0 pour ceux qui satisfont la condition, index 1 pour les autres. Le IF ne filtre pas, il aiguille : la somme des deux branches vaut le nombre d'items entres. ⚠️ Si la branche « faux » n'est branchee nulle part, ses items s'arretent la, en silence. Trois tests le verifient.
      //     passage.itemsSortis += sortants.length; return;
      const vrais = sortants.filter(decision);
      const faux = sortants.filter((i) => !decision(i));
      passage.itemsSortis += vrais.length + faux.length;
      const sorties = [enfantsDirects(flux, nom, 'main')];
      // Les deux sorties du IF, dans l'ordre : vrai puis faux.
      const liens = (flux.connectionsBySourceNode || {})[nom] || {};
      const branches = (liens.main || []).map((b) => b.map((l) => l.node));
      for (const cible of branches[0] || []) {
        visiter(cible, vrais, new Set(vus).add(nom));
      }
      for (const cible of branches[1] || []) {
        visiter(cible, faux, new Set(vus).add(nom));
      }
      // ⚠️ Les items de la branche « faux » quand elle n'est pas branchée
      // ne vont nulle part. Ils ne sont pas « perdus » au sens d'une erreur :
      // ils s'arrêtent là, en silence.
      if (!(branches[1] || []).length) passage.itemsSortis -= faux.length;
      return;
      // <<<
    }
