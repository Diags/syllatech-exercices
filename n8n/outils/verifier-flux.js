...
function verifier(donnees, graphe) {
  ...
  for (const { noeud, chemin, valeur } of parametres(donnees)) {
    ...
    // >>> depart: pour chaque $('Nom') d'une expression, verifier que le noeud EXISTE, puis qu'il est en AMONT de celui qui le lit. Un noeud en aval n'a pas encore de donnees quand l'expression s'evalue, et n8n ne le signale qu'a l'execution — jamais au chargement. Deux tests le verifient.
    //     continue;
    for (const cible of noeudsReferences(valeur)) {
      if (!existants.has(cible)) {
        soucis.push(souci('erreur', `${noeud} / ${chemin}`,
          `reference $('${cible}') : ce noeud n'existe pas. L'expression `
          + 'rendra une erreur a l\'execution, pas au chargement.'));
        continue;
      }
      const parents = graphe.getParentNodes(noeud, 'main');
      if (!parents.includes(cible) && cible !== noeud) {
        soucis.push(souci('erreur', `${noeud} / ${chemin}`,
          `reference $('${cible}') : ce noeud n'est pas en AMONT de `
          + `« ${noeud} ». Ses donnees n'existent pas encore quand `
          + `« ${noeud} » s'execute.`));
      }
    }
    // <<<
