...
def saturer(cible: str = "memory", taille: int = 60,
    ...
    """
    # >>> depart: ecrire dans le magasin jusqu'a ce que Hermes REFUSE, et rendre (magasin, nombre accepte, refus). La capacite ne se deduit pas du plafond : une entree coute un peu plus que son texte. Trois tests le verifient, dont un sur le contenu du refus — qui est une consigne de consolidation, pas un message d'erreur.
    #     return neuf(), 0, Ecriture(False, "a ecrire", "", 0)
    store = neuf()
    acceptees = 0
    for numero in range(maximum):
        ecriture = ecrire(store, cible, f"Fait {numero:03d} : " + "x" * taille)
        if not ecriture.reussie:
            return store, acceptees, ecriture
        acceptees += 1
    raise AssertionError(f"{maximum} ecritures acceptees sans jamais de refus")
    # <<<
