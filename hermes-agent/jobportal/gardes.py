...
def juger(commande: str) -> Verdict:
    ...
    # >>> depart: interroger les deux detecteurs de tools.approval et rendre un Verdict. ⚠️ Ils n'ont PAS la meme forme : detect_dangerous_command rend trois valeurs (bool, raison, categorie), detect_hardline_command en rend deux. Les depaqueter pareil leve un ValueError. Douze tests le verifient.
    #     return Verdict(commande, False, "", False, "")
    dangereuse, raison_d, _categorie = approval.detect_dangerous_command(commande)
    hardline, raison_h = approval.detect_hardline_command(commande)
    return Verdict(commande=commande,
                   dangereuse=bool(dangereuse), raison_dangereuse=raison_d or "",
                   hardline=bool(hardline), raison_hardline=raison_h or "")
    # <<<
