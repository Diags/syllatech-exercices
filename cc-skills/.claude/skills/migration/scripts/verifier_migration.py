...
def verifier(texte: str) -> list[str]:
    ...
    # >>> depart: refuser une migration sans section « -- down ». C'est l'invariant que la skill grave : une migration sans rollback passe les tests, part en production, et ne se remarque que le jour ou il faut revenir en arriere.
    #     pass
    if not re.search(r"^\s*--\s*down\b", texte, re.IGNORECASE | re.MULTILINE):
        soucis.append("section « -- down » absente : migration irreversible")
    # <<<
