...
def executer_commandes(corps: str, dossier: Path) -> tuple[str, list[tuple[str, int]]]:
    ...
    """
    # TODO : remplacer chaque bloc !`commande` par la SORTIE de la commande, et relever son cout. subprocess.run(commande, shell=True, cwd=dossier, stdin=subprocess.DEVNULL, capture_output=True, text=True, timeout=20) — stdin FERME, comme au vrai rendu. Rendre (texte rendu, [(commande, tokens(sortie)), ...]). Trois tests le verifient.
    return corps, []
