...
def verifier_une(dossier: Path) -> list[Souci]:
    ...
    # >>> depart: signaler les champs de frontmatter absents de CHAMPS_CONNUS. « descriptio » au lieu de « description » est ignore en SILENCE : la skill ne se declenche jamais et rien ne le dit.
    #     pass
    for champ in champs:
        if champ not in CHAMPS_CONNUS:
            soucis.append(Souci("attention", nom_dossier,
                                f"champ inconnu « {champ} » : il sera ignoré "
                                "en silence. Faute de frappe ?"))
    # <<<
    ...
    # >>> depart: signaler tout script lance par un chemin RELATIF. Les chemins qui partent de ${CLAUDE_SKILL_DIR}, ${CLAUDE_PLUGIN_ROOT}, / ou ~ sont corrects. Le test test_le_chemin_relatif_du_cours_est_attrape exige que le message cite CLAUDE_SKILL_DIR.
    #     pass
    for m in re.finditer(r"(?:^|\s)((?:node|python|python3|bash|sh)\s+([^\s`]+\.(?:js|mjs|py|sh)))",
                         corps):
        chemin = m.group(2)
        if chemin.startswith(("${CLAUDE_SKILL_DIR}", "${CLAUDE_PLUGIN_ROOT}", "/", "~")):
            continue
        soucis.append(Souci("erreur", nom_dossier,
                            f"« {m.group(1).strip()} » : chemin relatif au dossier de "
                            f"travail, pas à la skill. Écrivez "
