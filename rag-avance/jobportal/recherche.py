...
def rrf(listes: list[list[Morceau]], k: int = 60) -> list[Morceau]:
    ...
    """
    # >>> depart: ecrire la fusion RRF. Elle ne doit regarder que les RANGS, jamais les scores : ceux de deux moteurs ne sont pas comparables.
    #     return []
    scores: dict[str, float] = {}
    par_id: dict[str, Morceau] = {}
    for liste in listes:
        for rang, m in enumerate(liste):
            scores[m.id] = scores.get(m.id, 0.0) + 1 / (k + rang)
            par_id[m.id] = m
    return [par_id[i] for i in sorted(scores, key=lambda i: scores[i], reverse=True)]
    # <<<
