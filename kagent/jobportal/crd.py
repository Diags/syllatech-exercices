...
def valider(schema: dict[str, Any], objet: Any,
    ...
    if attendu == "object":
        ...
        # >>> depart: valider chaque propriete connue, et ELAGUER les autres
        #     return objet, problemes, elagues
        for nom, valeur in objet.items():
            sous_chemin = f"{chemin}.{nom}" if chemin else nom
            if nom not in proprietes:
                # ⚠️ L'ELAGAGE. L'API ne dit rien, elle retire.
                elagues.append(sous_chemin)
                continue
            sous, sous_problemes, sous_elagues = valider(
                proprietes[nom], valeur, sous_chemin)
            resultat[nom] = sous
            problemes.extend(sous_problemes)
            elagues.extend(sous_elagues)

        # ⚠️ Le schema AJOUTE aussi, et tout aussi discretement : les
        # valeurs par defaut sont ecrites dans l'objet stocke, alors
        # qu'elles n'apparaissent nulle part dans le fichier envoye.
        for nom, sous_schema in proprietes.items():
            if nom not in resultat and "default" in sous_schema:
                resultat[nom] = sous_schema["default"]

        for exige in schema.get("required") or []:
            if exige not in resultat:
                sous_chemin = f"{chemin}.{exige}" if chemin else exige
                problemes.append(Probleme(sous_chemin, "champ obligatoire absent"))

        for nom in schema.get("x-kubernetes-validations") or []:
            if nom not in REGLES:
                raise ErreurSchema(f"regle inconnue : « {nom} »")
            predicat, message = REGLES[nom]
            if not predicat(resultat):
                problemes.append(Probleme(chemin or ".", message))

        return resultat, problemes, elagues
        # <<<
