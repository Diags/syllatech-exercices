...
# >>> depart: exposer une offre en RESOURCE, pas en outil. L'URI porte un paramètre ; le décorateur le passe à la fonction. Pourquoi une resource et pas un outil ? Le README le dit.
#     def offre(offre_id: str) -> str:
#         """Expose une offre en lecture."""
#         return donnees.get_offre(offre_id)
@serveur.resource("offres://{offre_id}")
def offre(offre_id: str) -> str:
    """Expose une offre en lecture. Donnée, pas action : un client peut la
    lire sans rien changer, donc sans avoir à vous demander."""
    return donnees.get_offre(offre_id)
# <<<
