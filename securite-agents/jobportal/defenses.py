...
def construire_prompt(cv: str, poste: str, delimite: bool) -> tuple[str, tuple[int, int]]:
    ...
    suffit d'une fois.
    """
    # >>> depart: rendre le prompt ET la zone (debut, fin) qu'occupe le CV dedans. La zone doit etre exacte : prompt[debut:fin] == cv, avec ou sans delimiteurs. Deux tests le verifient.
    #     return GABARIT_NU.format(poste=poste, cv=cv), (0, 0)
    gabarit = GABARIT_DELIMITE if delimite else GABARIT_NU
    avant = gabarit.split("{cv}", 1)[0].format(poste=poste)
    return gabarit.format(poste=poste, cv=cv), (len(avant), len(avant) + len(cv))
    # <<<
...
class Outils:
    ...
    def appeler(self, nom: str, arguments: dict) -> str:
        ...
        # >>> depart: appliquer les trois mecanismes, DANS CET ORDRE : liste blanche (Refus), role (Refus si RESERVES_RH et role != rh), irreversible (mettre en attente et NE PAS executer). Tracer chaque refus — un refus est un signal d'attaque. Cinq tests le verifient.
        #     pass
        if nom not in self.autorises:
            self.journal.tracer(self.role, f"refus:{nom}", arguments)
            raise Refus(f"« {nom} » n'est pas dans la liste blanche")

        if nom in self.RESERVES_RH and self.role != "rh":
            self.journal.tracer(self.role, f"refus_role:{nom}", arguments)
            raise Refus(f"« {nom} » est reserve au role RH")

        if nom in self.IRREVERSIBLES:
            self.en_attente.append((nom, arguments))
            self.journal.tracer(self.role, f"en_attente:{nom}", arguments)
            return f"Proposition #{len(self.en_attente)} — en attente de validation."
        # <<<
...
def anonymiser(texte: str) -> str:
    ...
    coordonnées.
    """
    # >>> depart: remplacer les adresses e-mail par [EMAIL] et les numeros de telephone par [TEL], SANS toucher au reste : un test verifie que « Java » et « Spring Boot » survivent. Une anonymisation qui detruit l'information utile ne sera pas adoptee.
    #     return texte
    return TELEPHONE.sub("[TEL]", EMAIL.sub("[EMAIL]", texte))
    # <<<
