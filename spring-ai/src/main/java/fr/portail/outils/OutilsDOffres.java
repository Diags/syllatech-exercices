 ...
 * Spring AI lit la signature, en déduit un <strong>schéma JSON</strong>, et
 * l'envoie au modèle avec le prompt. Le chapitre 4 imprime ce schéma — c'est
 * lui, et non la description, qui dit au modèle comment appeler la méthode.
 *
...
public class OutilsDOffres {
    ...
    @Tool(description = "Recherche les offres d'emploi du portail dont le "
            + "titre ou la description contient un mot-cle. Rend leurs "
            + "references et leurs villes.")
    public List<String> rechercherOffres(
            @ToolParam(description = "le mot-cle a chercher, en minuscules")
            String motCle) {
        APPELS.incrementAndGet();
        return fr.portail.rag.Corpus.OFFRES.stream()
                .filter(d -> d.getText().toLowerCase(java.util.Locale.ROOT)
                        .contains(motCle.toLowerCase(java.util.Locale.ROOT)))
                .map(d -> d.getMetadata().get("reference") + " a "
                        + d.getMetadata().get("ville"))
                .toList();
    }

    @Tool(description = "Rend le salaire minimum annonce pour une offre, "
            + "en euros par an.")
    public int salaireDe(
            @ToolParam(description = "la reference de l'offre, par exemple OFF-014")
            String reference) {
        APPELS.incrementAndGet();
        return fr.portail.rag.Corpus.OFFRES.stream()
                .filter(d -> reference.equals(d.getMetadata().get("reference")))
                .map(d -> (Integer) d.getMetadata().get("salaire"))
                .findFirst()
                .orElse(-1);
    }
     ...
     */
    @Tool(description = "Depose une candidature du candidat connecte sur une "
            + "offre, identifiee par sa reference.")
    public String postuler(
            @ToolParam(description = "la reference de l'offre") String reference) {
        APPELS.incrementAndGet();
        // >>> depart: refuser une reference qui n'existe pas, AVANT d'ecrire quoi que ce soit
        //     // sans cette verification, le modele cree ce qu'il veut
        boolean connue = fr.portail.rag.Corpus.OFFRES.stream()
                .anyMatch(d -> reference.equals(d.getMetadata().get("reference")));
        if (!connue) {
            return "refus : l'offre " + reference + " n'existe pas";
        }
        // <<<
