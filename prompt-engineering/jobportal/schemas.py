...
# >>> depart: décrire le contrat de sortie. Les tests exigent qu'un avis d'un mot soit REFUSÉ et qu'un risque de 42 le soit aussi : le schéma contraint le contenu, pas seulement la forme.
...
class Analyse(BaseModel):
    ...
    avis: str = Field(min_length=10, description="l'avis donné au candidat")
    risque: int = Field(ge=0, le=10, description="risque perçu, de 0 à 10")
    manques: list[str] = Field(default_factory=list,
                               description="les informations absentes de l'offre")
# <<<
