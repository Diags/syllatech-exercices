...
public class Candidature {
    ...
    public void traiter(PlanifierEntretien commande) {
        // >>> depart: refuser de planifier un entretien sur une candidature deja close
        //     // sans cette regle, on planifie un entretien sur une candidature annulee
        if (close) {
            throw new IllegalStateException(
                    "candidature close : plus d'entretien possible");
        }
        // <<<
    ...
    public void traiter(Decider commande) {
        ...
        // >>> depart: refuser de decider tant qu'aucun entretien n'a ete planifie
        //     // c'est l'invariant qui justifie l'existence de cet agregat
        // ⚠️ L'invariant qui justifie l'existence de l'agregat : on ne decide
        // pas d'un candidat qu'on n'a pas vu. C'est exactement le genre de
        // regle qu'une projection ne peut PAS garantir — elle est en retard.
        if (!entretienPlanifie) {
            throw new IllegalStateException(
                    "aucun entretien planifie : decision impossible");
        }
        // <<<
    ...
    public void traiter(AnnulerCandidature commande) {
        // >>> depart: rendre l'annulation IDEMPOTENTE — annuler deux fois ne doit produire qu'un seul evenement
