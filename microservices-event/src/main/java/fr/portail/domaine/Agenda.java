...
public class Agenda {
    ...
    public void traiter(ReserverCreneau commande) {
        ...
        // >>> depart: publier un EVENEMENT de refus quand il n'y a plus de creneau — pas une exception
        //     // une exception ne serait vue que de l'appelant : la saga resterait bloquee
        if (reserves.size() >= CRENEAUX) {
            // ⚠️ Un REFUS qui est un EVENEMENT, et non une exception. C'est
            // une decision metier : « plus de creneau » interesse la saga,
            // qui doit compenser. Une exception, elle, ne serait vue que de
            // l'appelant — et la saga resterait bloquee a attendre.
            apply(new CreneauRefuse(agendaId, commande.candidatureId(),
