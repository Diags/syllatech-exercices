...
class Graphe:
    ...
    def ajouter(self, fait: Fait) -> list[Fait]:
        ...
        """
        # >>> depart: fermer les faits que celui-ci contredit — meme sujet, meme predicat, objet DIFFERENT. Un fait identique ne ferme rien, sinon repeter une preference la remettrait a zero. Et si le fait arrive EN RETARD (valid_at plus ancien), c'est LUI qu'on ferme, sinon on produit un intervalle inverse. Six tests le verifient.
        #     self.faits.append(fait); return []
        fermes = []
        for ancien in self.faits:
            if (ancien.sujet != fait.sujet or ancien.predicat != fait.predicat
                    or ancien.objet == fait.objet):
                continue

            if ancien.valid_at <= fait.valid_at:
                # Le cas normal : le nouveau succède à l'ancien.
                if ancien.valide:
                    ancien.fermer(fait.valid_at)
                    fermes.append(ancien)
            else:
                # ⚠️ LE FAIT ARRIVE EN RETARD — il est plus ANCIEN que ce
                # qu'on a déjà. C'est le cas d'une ingestion d'historique dans
                # le désordre, et le traiter comme le cas normal produirait un
                # intervalle INVERSÉ : « valide du 17 juillet au 1er mars ».
                #
                # Un intervalle inversé n'est pas une donnée bizarre : il rend
                # `a_la_date()` faux pour toujours, sans lever la moindre
                # erreur. On place donc le fait en retard AVANT celui qu'on a,
                # et c'est lui qu'on ferme.
                if fait.invalid_at is None or fait.invalid_at > ancien.valid_at:
                    fait.fermer(ancien.valid_at)
        self.faits.append(fait)
        self.faits.sort(key=lambda f: f.valid_at)
        return fermes
        # <<<
    ...
    def a_la_date(self, sujet: str, quand: datetime) -> list[Fait]:
        ...
        """
        # >>> depart: rendre les faits du sujet qui etaient VALIDES a cette date — ouverts avant, et pas encore fermes. Deux tests le verifient.
        #     return []
        return [f for f in self.faits
                if f.sujet == sujet and f.valid_at <= quand
                and (f.invalid_at is None or f.invalid_at > quand)]
        # <<<
