...
class _Threads:
    ...
    def get_user_context(self, thread_id: str) -> Contexte:
        fil = self._m.threads[thread_id]
        # >>> depart: construire le bloc a partir des faits VALIDES seulement. Y mettre les faits fermes donnerait au modele deux versions contradictoires sans dire laquelle est actuelle — c'est exactement ce que fait une memoire naive. Trois tests le verifient.
        #     return Contexte("", [])
        faits = self._m.graphe.valides(self._nom(fil.user_id))
        if not faits:
            return Contexte("(aucun fait connu sur cet utilisateur)", [])
        lignes = [f"- {f.texte}" for f in faits]
        return Contexte("FAITS CONNUS SUR L'UTILISATEUR :\n" + "\n".join(lignes),
                        faits)
        # <<<
