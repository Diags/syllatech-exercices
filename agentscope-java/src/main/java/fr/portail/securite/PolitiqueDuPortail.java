...
public final class PolitiqueDuPortail {
    ...
    public static PermissionContextState lectureSeule() {
        // >>> depart: poser les regles du portail — le MODE, puis une regle par outil. Attention au contenu des regles : voir TOUS_LES_ARGUMENTS
        ...
        return PermissionContextState.builder()
                ...
                // ⚠️ DENY et non ASK : une approbation que personne ne
                // recueille produit un agent qui annonce « c'est fait » pour
                // une action qui n'a pas eu lieu. Tant que l'interface ne
                // traite pas `REQUIRE_USER_CONFIRM`, on refuse franchement.
                .addDenyRule("supprimer_offre",
                             regle("supprimer_offre", PermissionBehavior.DENY))
                .build();
        // <<<
    }
    ...
    public static boolean cheminAcceptable(String chemin) {
        ...
        // >>> depart: refuser tout chemin qui SORT du workspace — l'absolu comme la remontee. Resoudre et NORMALISER : comparer des chaines ne suffit pas, et sur exception on REFUSE
        //     return true;
        try {
            Path candidat = Path.of(chemin);
            if (candidat.isAbsolute()) {
                return false;
            }
            // ⚠️ LA SEULE FACON QUI TIENNE : resoudre, NORMALISER, puis
            // verifier la containment. Comparer des chaines ne marche pas —
            // « a/../../b » ne contient pas « .. » au debut, et sort quand
            // meme. C'est `normalize()` qui fait le travail.
            Path racine = Path.of("").toAbsolutePath().normalize();
            Path cible = racine.resolve(candidat).normalize();
            return cible.startsWith(racine);
        } catch (RuntimeException erreur) {
            // ⚠️ FAIL CLOSED. Un chemin que l'on ne sait pas analyser est un
            // chemin que l'on refuse : s'il passait sur exception, il
            // suffirait de faire echouer l'analyse pour contourner le
            // controle.
            return false;
        }
        // <<<
