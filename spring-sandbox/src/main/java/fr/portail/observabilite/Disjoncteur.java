...
public final class Disjoncteur {
    ...
    public <T> T appeler(Supplier<T> appel, Supplier<T> repli,
        ...
        // >>> depart: rendre le repli SANS appeler tant que le circuit est ouvert, et l'ouvrir apres `seuil` pannes CONSECUTIVES (un succes remet le compteur a zero)
        //     return appel.get();
        if (etat == Etat.OUVERT) {
            return repli.get();
        }
        T resultat = appel.get();
        if (estUnePanne.test(resultat)) {
            echecsConsecutifs++;
            if (echecsConsecutifs >= seuil) {
                etat = Etat.OUVERT;
            }
        } else {
            echecsConsecutifs = 0;
