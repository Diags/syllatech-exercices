...
def correspond(regle: str, outil: str) -> bool:
    ...
    """
    # TODO : faire correspondre la regle a l'outil par un GLOB (fnmatch), pas par une expression reguliere. Cinq tests parametres le verifient, dont celui qui exige que « mcp__jobportal__.* » ne corresponde PAS.
    return regle == outil
...
def ancree(regle: str) -> bool:
    ...
    """
    # TODO : rendre True seulement si la regle commence par « mcp__<serveur>__ » ou <serveur> ne contient aucun caractere de glob (* ? [ ]). Quatre tests parametres le verifient.
    return True
...
def verifier(dossier: Path) -> list[Souci]:
    ...
    # LE contrôle central.
    # TODO : signaler toute regle deny ou ask qui ne correspond a AUCUN outil reellement expose. C'est LE controle central : les noms MCP contiennent « _ », donc Claude Code est exempte du controle de faute de frappe et ne dit rien. Deux tests le verifient.
    pass
