...
class Bride(SousProcessus):
    ...
    def _environnement(self) -> dict:
        # >>> depart: ne laisser au sous-processus que le strict necessaire pour demarrer — PATH et SYSTEMROOT — et rien d'autre. Toute variable qui reste est lisible par le code du candidat, cles d'API comprises. ⚠️ Vider l'environnement ne vaut que ce que le LANCEUR y remet ensuite : sous « uv run », PYTHONUSERBASE reapparait. Un test le verifie sur la propriete qui compte, pas sur un compte exact.
        #     return dict(os.environ)
        # PATH reste nécessaire pour que l'interpréteur démarre sur certains
        # systèmes ; tout le reste part, y compris les clés d'API.
        #
        # ⚠️ Vider l'environnement ne vaut que ce que le LANCEUR y remet
        # ensuite. Mesuré ici : sous `uv run`, le sous-processus voit aussi
        # `PYTHONUSERBASE`, qu'aucune ligne de ce fichier n'a mise. Le test
        # vérifie donc la propriété qui compte — aucune variable sensible —
        # plutôt qu'un compte exact, qui dépendrait de qui lance le projet.
