...
class Service:
    ...
    def recevoir(self, soumission: Soumission) -> Evaluation | None:
        ...
        """
        # >>> depart: refuser AVANT d'executer quoi que ce soit si le code depasse le plafond, journaliser le refus, et rendre None. Refuser apres avoir demarre un sous-processus coute un sous-processus — sur 400 candidatures, c'est la difference entre un refus gratuit et 80 secondes de machine. Deux tests le verifient.
        #     pass
        if len(soumission.code) > self.taille_max:
            self.depot.refuser(soumission.nom,
                               f"code de {len(soumission.code)} signes, "
                               f"plafond {self.taille_max}")
            return None

        evaluation = self.evaluateur.evaluer(soumission)
        self.depot.enregistrer(evaluation)
        return evaluation
        # <<<
