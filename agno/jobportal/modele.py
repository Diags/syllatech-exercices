...
class ModeleFactice(Model):
    ...
    def invoke(self, messages=None, tools=None, **kwargs) -> ModelResponse:
        ...
        # >>> depart: rendre un ModelResponse (JAMAIS un dict : « 'dict' object has no attribute 'role' »). S'il y a des outils, qu'aucun retour d'outil n'est encore la, et que la question le merite, appeler un outil ; sinon repondre A PARTIR du dernier retour d'outil. Sept tests le verifient.
        #     return ModelResponse(role="assistant", content="")
        retours = [m.content for m in messages if m.role == "tool"]
        # Un vrai modèle n'appelle pas un outil à chaque tour : il l'appelle
        # quand la question le demande. Reproduire ce discernement compte —
        # sans lui, on ne pourrait pas mesurer ce qu'un appel d'outil coûte,
        # puisque toutes les questions coûteraient pareil.
        if tools and not retours and self._merite_un_outil(messages, tools):
            return self._appeler_un_outil(messages, tools)
        return ModelResponse(role="assistant",
                             content=self._repondre(messages, retours))
        # <<<
...
def _arguments(outil, messages) -> dict:
    ...
    # >>> depart: construire un argument par propriete du schema, en RESPECTANT son type (array -> liste, boolean -> True, integer -> 3, enum -> premiere valeur). Un tableau recu sous forme de chaine leve une ValidationError pydantic, meme pour les outils internes d'Agno.
    #     return {}
    arguments = {}
    for nom, champ in schema.items():
        arguments[nom] = _valeur(nom, champ, sujet, messages)
    return arguments
    # <<<
