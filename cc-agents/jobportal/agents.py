...
class Session:
    ...
    def _outils(self, agent: Agent, contexte: Contexte) -> dict:
        def garde(nom, fonction):
            def appel(*a, **k):
                # >>> depart: refuser l'appel si l'outil est absent du « tools » de l'agent (« * » veut dire : herite de tout). Lever OutilRefuse en nommant l'outil ET la liste. C'est LA garantie du chapitre 2 : trois tests la verifient.
                #     pass
                if "*" not in agent.outils and nom not in agent.outils:
                    raise OutilRefuse(
                        f"L'agent « {agent.nom} » a appele {nom}, absent de son "
                        f"« tools: {', '.join(agent.outils)} ». Appel refuse.")
                # <<<
                sortie = fonction(*a, **k)
...
def fan_out(session: "Session", taches: list[tuple[str, str, object]]) -> list[Rapport]:
    ...
    # >>> depart: lancer les taches EN MEME TEMPS avec un ThreadPoolExecutor et rendre les rapports dans l'ordre des taches. Deux tests le verifient : le gain de temps, et la separation des contextes.
    #     return [session.deleguer(n, t, w) for n, t, w in taches]
    with ThreadPoolExecutor(max_workers=len(taches)) as pool:
        futurs = [pool.submit(session.deleguer, nom, tache, travail)
                  for nom, tache, travail in taches]
        return [f.result() for f in futurs]
    # <<<
