...
def verifier(constat: dict) -> tuple[str, str]:
    ...
    # >>> depart: relire la ligne et rendre un verdict SANS consulter le motif qui a produit le constat. identifiant : placeholder (EXAMPLE, CHANGEME...) -> douteux, valeur < 8 signes -> ecarte, sinon confirme. sql : deux litteraux concatenes -> ecarte, sinon confirme. temps : confirme. Tout le reste : ecarte. Six tests le verifient.
    #     return "confirme", ""
    if constat["genre"] == "identifiant":
        valeur = code.split("=", 1)[-1].strip().strip("\"'")
        if any(p in valeur.upper() for p in PLACEHOLDERS):
            return "douteux", ("valeur publique de documentation, pas un secret "
                               "fuite — mais elle n'a rien a faire la non plus")
        if len(valeur) < 8:
            return "ecarte", "valeur trop courte pour etre un secret"
        return "confirme", f"« {valeur} » est un secret exploitable, en clair dans le depot"

    if constat["genre"] == "sql":
        # La preuve indépendante : ce qui est concaténé porte un nom de
        # variable, donc vient de l'extérieur. Une concaténation de deux
        # littéraux serait laide, pas dangereuse.
        if re.search(r"[\"']\s*\+\s*[\"']", code):
            return "ecarte", "concatenation de deux litteraux : laid, pas dangereux"
        return "confirme", "une valeur externe entre dans la requete sans echappement"

    if constat["genre"] == "temps":
        return "confirme", "la comparaison s'arrete au premier octet different"

    return "ecarte", "preference de style : aucune consequence nommee"
    # <<<
