...
class Route:
    ...
    def correspondances(self) -> list[dict[str, Any]]:
        ...
        """
        # >>> depart: rendre `matches` s'il est ecrit, sinon le defaut du schema
        #     return self.brut.get("matches") or []
        valeur = self.brut.get("matches")
        return valeur if isinstance(valeur, list) and valeur \
            else CORRESPONDANCE_PAR_DEFAUT
        # <<<
