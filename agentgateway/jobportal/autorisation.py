...
def decider(regles: list[Regle], contexte: dict[str, Any]) -> Verdict:
    ...
    # >>> depart: composer les trois formes — tout `require` vrai, aucun `deny` vrai, un `allow` vrai
    #     return Verdict(True, "a completer", evaluations)
    for regle, resultat in evaluations:
        if regle.forme == "require" and resultat is not True:
            motif = ("require faux" if resultat is False
                     else f"require illisible ({resultat})")
            return Verdict(False, f"{motif} : {regle.expression}", evaluations)

    for regle, resultat in evaluations:
        if regle.forme == "deny" and resultat is True:
            return Verdict(False, f"deny vrai : {regle.expression}", evaluations)

    permissions = [(r, v) for r, v in evaluations if r.forme == "allow"]
    if not permissions:
        return Verdict(True, "aucune regle allow : rien ne s'y oppose",
                       evaluations)
    for regle, resultat in permissions:
        if resultat is True:
            return Verdict(True, f"allow vrai : {regle.expression}", evaluations)
    return Verdict(False, "aucune regle allow n'est vraie", evaluations)
    # <<<
