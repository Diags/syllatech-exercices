...
def choisir(routage: dict[str, Any], contexte: dict[str, Any]) -> Decision:
    ...
    # >>> depart: appliquer les trois formes — conditional, failover, weighted
    #     return Decision("", "a completer")
    conditionnel = routage.get("conditional")
    if isinstance(conditionnel, dict):
        cibles = conditionnel.get("targets") or []
        for rang, cible in enumerate(cibles):
            if not isinstance(cible, dict):
                continue
            expression = cible.get("when")
            # « Omit only on the final fallback target. » Une cible sans
            # `when` prend donc tout ce qui lui arrive.
            if expression is None:
                return Decision(cible.get("model", ""), "conditional",
                                "(repli)", rang)
            if evaluer(expression, contexte):
                return Decision(cible.get("model", ""), "conditional",
                                expression, rang)
        raise ErreurDeCEL("aucune cible ne correspond et aucun repli n'existe")

    bascule = routage.get("failover")
    if isinstance(bascule, dict):
        cibles = [c for c in bascule.get("targets") or [] if isinstance(c, dict)]
        if not cibles:
            raise ErreurDeCEL("failover sans cible")
        meilleure = min(cibles, key=lambda c: c.get("priority", 0))
        return Decision(meilleure.get("model", ""), "failover",
                        f"priority={meilleure.get('priority', 0)}",
                        cibles.index(meilleure))

    pondere = routage.get("weighted")
    if isinstance(pondere, dict):
        cibles = [c for c in pondere.get("targets") or [] if isinstance(c, dict)]
        if not cibles:
            raise ErreurDeCEL("weighted sans cible")
        total = sum(c.get("weight", 1) for c in cibles)
        premiere = cibles[0]
        part = premiere.get("weight", 1) / total if total else 0
        return Decision(premiere.get("model", ""), "weighted",
                        f"{part:.0%} du trafic", 0)

    raise ErreurDeCEL("routing sans weighted, failover ni conditional")
    # <<<
