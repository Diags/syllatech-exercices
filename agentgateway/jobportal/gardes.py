...
def appliquer(bloc_regex: dict[str, Any], texte: str) -> Constat:
    ...
    # >>> depart: appliquer chaque motif, et masquer quand l'action le demande
    #     return Constat(False, action, [], texte)
    touches = []
    masque = texte
    for regle in (bloc_regex or {}).get("rules") or []:
        if not isinstance(regle, dict):
            continue
        etiquette, motif = motif_de(regle)
        if not motif:
            continue
        if re.search(motif, texte):
            touches.append(etiquette)
            masque = re.sub(motif, "***", masque)
    return Constat(bool(touches), action, touches,
                   masque if action == "mask" else texte)
    # <<<
