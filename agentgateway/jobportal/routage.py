...
def chemin_correspond(regle: Any, chemin: str) -> bool:
    ...
    if not isinstance(regle, dict):
        return False
    # TODO : implanter les trois formes — exact, pathPrefix (par SEGMENT), regex
    return False
...
def correspondance_correspond(regle: dict[str, Any],
    ...
    """
    echecs = []
    # TODO : exiger que TOUS les champs poses passent — chemin, methode, en-tetes, parametres
    return True, []
...
def route_correspond(route: Route, requete: Requete) -> tuple[bool, list[str]]:
    ...
    raisons = []
    # TODO : un seul `matches` qui passe suffit — c'est un OU
