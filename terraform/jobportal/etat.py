...
class Verrou:
    ...
    def prendre(self, qui: str = "moi") -> None:
        # >>> depart: refuser si le verrou existe deja, sinon l'ecrire avec son detenteur
        #     pass
        if self.chemin.exists():
            detenteur = json.loads(self.chemin.read_text(encoding="utf-8"))
            raise EtatVerrouille(
                f"l'etat est verrouille par « {detenteur.get('who')} » "
                f"depuis {detenteur.get('created')}")
        self.chemin.parent.mkdir(parents=True, exist_ok=True)
