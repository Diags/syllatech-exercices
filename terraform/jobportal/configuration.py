...
def _instances(bloc: hcl.Bloc, contexte: dict[str, Any],
    ...
    # >>> depart: derouler `for_each` (adresse par CLE) et `count` (adresse par POSITION)
    #     return [InstanceVoulue(type_ressource, nom, None,
    #                            _attributs(bloc, contexte), empeche, module)]
    if "for_each" in bloc.attributs:
        collection = evaluer(bloc.attributs["for_each"], contexte)
        cles = (sorted(collection.keys()) if isinstance(collection, dict)
                else sorted(str(element) for element in collection))
        instances = []
        for cle in cles:
            valeur = (collection[cle] if isinstance(collection, dict) else cle)
            local = {**contexte, "each": {"key": cle, "value": valeur}}
            instances.append(InstanceVoulue(
                type_ressource, nom, cle,
                _attributs(bloc, local), empeche, module))
        return instances

    if "count" in bloc.attributs:
        combien = int(evaluer(bloc.attributs["count"], contexte))
        instances = []
        for rang in range(combien):
            local = {**contexte, "count": {"index": rang}}
            instances.append(InstanceVoulue(
                type_ressource, nom, str(rang),
                _attributs(bloc, local), empeche, module))
        return instances
