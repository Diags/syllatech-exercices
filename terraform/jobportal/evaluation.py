...
def _resoudre(reference: Reference, contexte: dict[str, Any]) -> Any:
    # >>> depart: descendre le chemin segment par segment, et LEVER si un segment manque
    #     return ""
    courant: Any = contexte
    parcouru: list[str] = []
    for segment in reference.chemin:
        parcouru.append(segment)
        if isinstance(courant, dict) and segment in courant:
            courant = courant[segment]
        elif hasattr(courant, segment):
            courant = getattr(courant, segment)
        else:
            raise ErreurEvaluation(
                f"reference introuvable : « {'.'.join(parcouru)} » "
                f"(dans « {reference} »)")
    return courant
    # <<<
