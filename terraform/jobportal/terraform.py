...
class Terraform:
    ...
    def apply(self, valeurs: dict[str, Any] | None = None,
        ...
        with Verrou(self.chemin_etat):
            # >>> depart: executer en trois temps — destructions, puis creations, puis modifications
            #     etat.ecrire()
            # 1. les destructions, d'abord — un remplacement doit liberer
            #    son nom avant de le reprendre.
            for changement in a_faire.changements:
                if changement.action in (Action.DETRUIRE, Action.REMPLACER):
                    identifiant = changement.avant.get("id")
                    if identifiant:
                        realite.detruire(identifiant)
                    adresse = changement.adresse.split("[")[0]
                    etat.retirer(adresse, changement.cle)

            # 2. les creations et les remplacements.
            for changement in a_faire.changements:
                if changement.action in (Action.CREER, Action.REMPLACER):
                    instance = changement.instance
                    assert instance is not None
                    complets = realite.creer(instance.type, instance.attributs)
                    etat.poser(instance.adresse, instance.cle, complets)

            # 3. les modifications en place.
            for changement in a_faire.changements:
                if changement.action is Action.MODIFIER:
                    instance = changement.instance
                    assert instance is not None
                    complets = realite.modifier(changement.avant["id"],
                                                instance.attributs)
                    etat.poser(instance.adresse, instance.cle, complets)
