...
class Composant:
    ...
    def effective(self) -> float:
        ...
        # >>> depart: rendre la disponibilite de N repliques en parallele
        #     return self.disponibilite
        return 1.0 - (1.0 - self.disponibilite) ** self.repliques
        # <<<
...
class Chaine:
    ...
    def disponibilite(self) -> float:
        # >>> depart: composer les disponibilites d'une chaine traversee de bout en bout
        #     return min(c.effective for c in self.composants)
        if not self.composants:
            raise ErreurDisponibilite(f"« {self.nom} » : chaine vide")
        resultat = 1.0
        for composant in self.composants:
            resultat *= composant.effective
        return resultat
        # <<<
