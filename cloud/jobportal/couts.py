...
def ajuster(observation: Observation, marge: float = 2.0) -> str:
    ...
    """
    # >>> depart: choisir le plus petit gabarit qui tient la POINTE, avec la marge
    #     return observation.gabarit
    actuelle = machine(observation.gabarit)
    besoin_vcpu = actuelle.vcpu * observation.cpu_pointe * marge
    besoin_memoire = actuelle.memoire_gio * observation.memoire_moyenne * marge
    candidats = [m for m in tarifs.MACHINES.values()
                 if m.vcpu >= besoin_vcpu and m.memoire_gio >= besoin_memoire]
    if not candidats:
        return observation.gabarit
    return min(candidats, key=lambda m: m.euros_par_heure).nom
    # <<<
...
def cout_sur_la_duree(gio: float, mois: int,
    ...
    # >>> depart: appliquer la regle en vigueur au mois considere, mois par mois
    #     return cout_stockage(gio, "standard", mois)
    total = 0.0
    for mois_ecoule in range(mois):
        jours = mois_ecoule * 30
        palier = "standard"
        for regle in sorted(regles or [], key=lambda r: r.jours):
            if jours >= regle.jours:
                palier = regle.palier
        total += cout_stockage(gio, palier, 1.0)
    return total
    # <<<
