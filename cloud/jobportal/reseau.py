...
class SousReseau:
    ...
    def utilisables(self) -> int:
        ...
        # >>> depart: retirer les adresses que le fournisseur se reserve
        #     return self.total
        return max(0, self.total - RESERVEES_PAR_SOUS_RESEAU)
        # <<<
...
class Vpc:
    ...
    def ajouter(self, sous_reseau: SousReseau) -> None:
        # >>> depart: refuser un sous-reseau hors du VPC, ou qui en chevauche un autre
        #     self.sous_reseaux.append(sous_reseau)
        if not sous_reseau.plage.subnet_of(self.plage):
            raise ErreurReseau(
                f"« {sous_reseau.nom} » ({sous_reseau.plage}) n'est pas "
                f"contenu dans le VPC {self.plage}")
        for existant in self.sous_reseaux:
            if sous_reseau.plage.overlaps(existant.plage):
                raise ErreurReseau(
                    f"« {sous_reseau.nom} » ({sous_reseau.plage}) chevauche "
                    f"« {existant.nom} » ({existant.plage})")
        self.sous_reseaux.append(sous_reseau)
        # <<<
