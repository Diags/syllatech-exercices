...
class Question:
    ...
    def bonne_reponse(self, critere: str) -> Option:
        # >>> depart: classer les options sur l'axe du critere, et refuser une egalite
        #     return self.options[0]
        axe, sens = _critere(critere)
        classees = sorted(self.options, key=lambda o: sens * o.note(axe),
                          reverse=True)
        meilleure, suivante = classees[0], classees[1]
        if meilleure.note(axe) == suivante.note(axe):
            raise ErreurExamen(
                f"deux options sont a egalite sur « {axe} » : "
                f"{meilleure.lettre} et {suivante.lettre} — la question est "
                f"mal posee")
        return meilleure
        # <<<
...
def detecter(enonce: str) -> str:
    ...
    """
    # >>> depart: trouver le mot qui tranche, et refuser un enonce qui n'en a pas
    #     return "le plus economique"
    normalise = re.sub(r"\s+", " ", enonce.lower())
    trouves = [critere for motif, critere in _MARQUEURS
               if re.search(motif, normalise)]
    trouves += [critere for critere in CRITERES
                if critere in normalise and critere not in trouves]
    if not trouves:
        raise ErreurExamen(
            "aucun mot qui tranche dans l'enonce — relisez-le : « le plus "
            "economique », « le plus disponible », « le plus securise », "
            "« le moins d'administration »")
    if len({CRITERES[c][0] for c in trouves}) > 1:
        raise ErreurExamen(
            f"deux criteres contradictoires dans le meme enonce : {trouves}")
    return max(trouves, key=len)
    # <<<
